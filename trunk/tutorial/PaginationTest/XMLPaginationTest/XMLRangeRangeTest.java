...
public class XMLRangeRangeTest extends fit.Fixture
{
	private int httpStatus, maxRange, count, rangeStart, rangeSize, pageNum, rangeSt, rangeSi, pageN, countN;
	...
	public boolean PageNumber()
	{
		...
		try
		{
			...
			{
				if(httpStatus==200)
					pageN = (rangeSt/rangeSi)+1;
				else if(httpStatus!=200)
					pageN=0;
			}
			else if(maxRange!=0)
				pageN = (rangeSt+rangeSi)/rangeSi;
			if(pageN == pageNum)
				var = true;
		}
		catch (Exception e)
